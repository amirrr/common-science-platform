...
export const explanationFormSchema = z.object({
  ...
  explanationText: z.string().max(500, {
  ...
  const form = useForm<ExplanationFormValues>({
    ...
    defaultValues: existingResponse
      ? {
          ...
          rating: existingResponse.rating !== undefined ? existingResponse.rating : 3,
      ...
      <CardContent>
        {existingResponse && (
          <div className="mb-6 p-3 bg-secondary/50 border border-border rounded-md flex items-center text-sm text-muted-foreground">
            ...
            <span>You have already submitted an explanation for this correlation. Your previous answers are shown below and cannot be changed.</span>
          </div>
        )}
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  <FormLabel className="text-lg font-medium">Which explanation seems most plausible for the observed correlation?</FormLabel>
                  <FormControl>
                    <RadioGroup
                    ...
                    >
                      {explanationsToList.map((explanation) => (
                        <FormItem key={explanation.id} className="flex items-start space-x-3 space-y-0 p-3 rounded-md border border-input hover:bg-accent/10 transition-colors data-[disabled]:opacity-70 data-[disabled]:hover:bg-transparent">
                          <FormControl>
                            <RadioGroupItem
            ...
            <FormField
              control={form.control}
              name="rating"
              render={({ field: { onChange, value, disabled, ...fieldProps } }) => (
                <FormItem>
                  <FormLabel className="text-lg font-medium">Rate your confidence in your chosen explanation (1-5): {value}</FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-lg font-medium">Provide your reasoning or any other insights (Optional):</FormLabel>
                  <FormControl>
                    <Textarea
            ...
            />
            {!existingResponse && (
              <Button type="submit" disabled={isSubmitting || form.formState.disabled} size="lg" className="w-full sm:w-auto">
                {isSubmitting ? (
                  <>
