...
export interface BidirectionalExplanation {
  type1: string; // "Common" or baseline explanation
  type2: string; // Explains A -> B
  type3: string; // Explains B -> A
}
 ...
 * Group X sees:
 * - Expl 1 (Baseline)
 * - Expl 2 (A -> B direction)
 * - Expl 3 (B -> A direction)
 *
 * Group Y sees:
 * - Expl 1 (Baseline)
 * - Expl 2 (B -> A direction)
 * - Expl 3 (A -> B direction)
 */
export function getExplanationsForGroup(
  correlation: BidirectionalCorrelation,
  group: "X" | "Y",
) {
  // Always use the explanations matching the current group's direction
  const directionData =
