...
export default function CRTTestPage() {
  ...
  const saveCRTToFirestore = async (crtData: CRTData) => {
    if (!userId) {
      toast({
        title: "Error",
        description: "User ID missing. Cannot save CRT to database.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Save Error",
        description: `Could not save CRT answers to database. They're saved locally. ${
    ...
    <div className="min-h-screen flex flex-col items-center bg-background text-foreground p-4 md:p-8">
      <header className="w-full max-w-xl sticky top-0 bg-background/80 backdrop-blur-md py-4 z-10 mb-8">
        ...
        <p className="text-sm text-muted-foreground text-center">
          Cognitive Reflection Test (Overall task {currentOverallStep} of{" "}
          {totalStudyParts})
        </p>
      </header>
              ...
              >
                {CRT_QUESTIONS.map((q) => (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor={q.id} className="font-semibold">
                          {q.label}
                        </FormLabel>
                ...
                >
                  {isSubmitting
                    ? "Submitting..."
                    : "Submit Answers & Proceed to Demographics"}
      ...
      <footer className="w-full max-w-4xl mt-12 pt-8 border-t text-center text-muted-foreground text-sm">
        <p>
          &copy; {new Date().getFullYear()} Correlation Study. Your responses
          are valuable.
        </p>
      </footer>
    </div>
